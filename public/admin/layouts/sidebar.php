<!-- sidebar -->

<div class="submenu text-right lh-md" style="margin-top: 8vh;">
    <i class="bi bi-person-fill ms-3 me-3 text-light"></i><a href="index.php"
        class="text-decoration-none text-light fs-5">Candidate Data</a><br><br>
    <i class="bi bi-clipboard2-data ms-3 me-3 text-light"></i><a href="ballot.php"
        class="text-decoration-none text-light fs-5">Ballot Data</a><br><br>

    <a href="../functions/logout.php" class="text-decoration-none mb-5 text-light fs-5 fixed-bottom" style="width:200px;"><i
            class="bi bi-box-arrow-left ms-3 me-2 text-light"></i>Logout</a><br><br>
</div>

<!-- end sidebar -->
